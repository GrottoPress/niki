struct Niki::Message::Endpoint
  include Niki::Endpoint

  def initialize(@client : Niki)
  end

  def input_items : InputItem::Endpoint
    InputItem::Endpoint.new(@client)
  end

  def fetch(id : String, headers = nil) : Item
    path = "#{uri.path}/#{id}"
    response = @client.get(path, headers)

    Item.from_json(response)
  end

  def create(headers = nil, **params) : Item
    response = @client.post(uri.path, headers, params.to_json)
    Item.from_json(response)
  end

  def delete(id : String, headers = nil) : Item
    path = "#{uri.path}/#{id}"
    response = @client.delete(path, headers)

    Item.from_json(response)
  end

  def input_tokens(headers = nil, **params) : Usage::Item
    path = "#{uri.path}/input_tokens"
    response = @client.post(path, headers, params.to_json)

    Usage::Item.from_json(response)
  end

  def cancel(id : String, headers = nil) : Item
    path = "#{uri.path}/#{id}/cancel"
    response = @client.post(path, headers)

    Item.from_json(response)
  end

  def compact(headers = nil, **params) : Item
    path = "#{uri.path}/compact"
    response = @client.post(path, headers, params.to_json)

    Item.from_json(response)
  end

  getter uri : URI do
    URI.parse(@client.uri.to_s).tap { |uri| uri.path += "/responses" }
  end
end
